...
| 28 | Unsupported operators (With, Distinct, set ops, subqueries incl. `IN (SELECT…)`, windows, BETWEEN/LIKE, arithmetic exprs, AVG/MIN/MAX, FULL/CROSS join, …) | `parse_query` / `_parse_predicate` / `_parse_select_expr` raise `ValueError` → status `error` | N/A — fail-closed by design; never silently dropped, so no false-equivalent path. (OR / NOT / `IN (value-list)` are now **supported** — see row 10.) |
