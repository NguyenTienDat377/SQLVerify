...
| 10 | Predicate semantics, three-valued logic (Fig. 9, §3.3, §4.4.3) | `_pred_true()`: filters keep a row only when the predicate is TRUE; any comparison with a NULL operand is non-TRUE; `IS [NOT] NULL` are the only NULL-tolerant predicates; column-vs-column requires both sides non-NULL | MATCHES |
| 11 | E-Filter (Fig. 12) | `where_all()` conjoined into `pair_present` / `single_present` / `nullext_*_present` | MATCHES — output tuple "deleted" iff input deleted or predicate not TRUE |
...
| 28 | Unsupported operators (With, Distinct, set ops, subqueries, windows, OR/IN/BETWEEN/LIKE, arithmetic exprs, AVG/MIN/MAX, multi-join, …) | `parse_query` / `_parse_conditions` / `_parse_select_expr` raise `ValueError` → status `error` | N/A — fail-closed by design; never silently dropped, so no false-equivalent path |
