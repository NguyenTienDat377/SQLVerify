...
| **CSS is fingerprinted, never hand-versioned** | `base.html`/`landing.html` link `styles.css?v={{ static_v('css/styles.css') }}` — an 8-char content hash from `_asset_version()` in `main.py`. **This exists because of a real outage:** the HTML is dynamic and uncached while `/static` sits behind Cloudflare at `max-age=14400`, so a deploy served new markup against a four-hour-old stylesheet. The workspace grid had gained a resizer column; the stale sheet still declared three tracks, so the results panel wrapped to row 2 and rendered 250px wide. Both files were individually correct. A hashed URL is a new cache key, so the edge must refetch — no purge, no waiting out the TTL. Never replace it with a hand-bumped `?v=1.1` (landing.html had one; it only works if someone remembers). The `@import`ed `tokens/*.css` can't carry the query — their URLs live inside the CSS text — so `/static/css/` is additionally served `max-age=0, must-revalidate`; fonts and images keep the long cache. |
...
- WHERE/HAVING are full boolean trees of `AND` / `OR` / `NOT` over comparisons and `IS [NOT] NULL`, encoded under three-valued (Kleene) logic per the VeriEQL grammar (Fig. 4) and filter rule (Fig. 5: a row survives iff the predicate is TRUE). `IN (value-list)` is supported (desugars to `= v1 OR = v2 …`); `NOT IN` and `NOT (…)` too. **`col IN (SELECT c FROM …)`** — the paper's `E⃗ ∈ Q` semi-join (Fig. 4) — is supported in WHERE for an **uncorrelated, single-column** body (any supported query: joins, GROUP BY, aggregates). The body is materialized once (`_parse_in_subquery` → `encode_query` → `cond.subquery_qf`) and encoded as a three-valued membership disjunction in `_pred_tf`; `NOT IN` is `NOT(IN)`, so the NOT-IN-NULL trap is handled soundly (a NULL body cell makes `NOT IN` never TRUE). **Fail-closed:** correlated bodies (outer-alias reference), scalar/`EXISTS` subqueries, tuple-LHS `(a,b) IN …`, multi-column bodies, and `IN (SELECT …)` in HAVING. Still rejected: `BETWEEN`, `LIKE`.
